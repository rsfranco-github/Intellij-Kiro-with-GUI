...
/**
 * KiroMessages 다국어 시스템 테스트.
 * KiroMessages는 내부적으로 KiroSettings.getInstance()를 호출하므로
 * IDE 컨텍스트 없이는 직접 호출 불가. 여기서는 메시지 맵 구조를 검증한다.
 */
class KiroMessagesTest {

    // KiroMessages.kt의 ko 맵과 동일하게 유지
    private val koMessages = mapOf(
        // 탭 이름
        "tab.chat" to "채팅",
        "tab.manage" to "관리",
    ...
    // KiroMessages.kt의 en 맵과 동일하게 유지
    ...
    fun `ko messages should contain Korean characters where expected`() {
        ...
        // 한글이 포함되어야 하는 키들 (MCP, Skills, Agent 등 영어 고유명사 제외)
    ...
    fun `ko and en messages should be different for translatable keys`() {
        // 번역되어야 하는 키들 (MCP, Skills, Agent 등 고유명사 제외)
    ...
    fun `total message count should match between ko and en`() {
        assertEquals(koMessages.size, enMessages.size, 
